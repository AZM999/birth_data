...
# List of month-wise births (or birth rate) sorted ascendingly month-wise
# from_, to  - years of data given (must be compete)
# quarterly  - to generate graph for qtrs
# monthly    - to generate graph for month-wise
# country    - name of the country
# br         - flag to specify if its birth_rate that is given in the data for calculations
#
#
## import libraries
...
def plot_bar (x_data, y_data, x_label, y_label, title, ):
    ...
    mp.bar_label (figure, labels=y_data, label_type='edge', rotation=60)
    mp.show ()
...
def generate_graph (data, from_, to, quarterly, monthly, country, br, title):
    ...
    if quarterly==1 or quarterly==True:
        ...
        plot_bar (qt, dat, 'Time period (Quarter)', 'Total ' + title, 
                  str(country)+' - '+title+' '+str(from_)+'-'+str(to)+' Quaterly')
